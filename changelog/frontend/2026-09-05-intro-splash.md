...
## 추가 수정 (`feature/intro-splash-style`)

- 영상 풀스크린 → 여백 있는 중앙 배치(rounded + shadow)
- '입장하기' 텍스트 필 → 브랜드 로고(파비콘 svg) + 플레이 아이콘
- 하단 컨트롤: 보더/라운드 제거, 텍스트 + 호버로 통일.
  '오늘 하루 보지 않기'는 체크박스 → 클릭 즉시 저장+닫기 버튼으로 변경

## 추가 수정 2 (스타일 반복, PR #20~#25)

- 영상 라운드 제거·컨트롤을 영상 하단 우측으로, 입장 버튼은 글래스 원형 플레이만 유지
- 재생 대기 오버레이를 영상 영역만 덮게 이동 → 재생 전에도 건너뛰기/오늘 하루 보지 않기 사용 가능
- 컨트롤 텍스트 최종 18px, 흰색 기본 + 브랜드 호버(text-brand-hover)

## 추가 수정 3 — 노출 규칙 (`feature/intro-show-rules`)

- 첫 진입·홈 새로고침에만 재생. GNB 홈 SPA 재진입/로그인·로그아웃 리다이렉트 복귀/
  뒤로가기에는 미표시 (Navigation Timing 진입 유형 + sessionStorage 세션 플래그
  + 문서 로드당 1회 판단). '오늘 하루 보지 않기'는 항상 우선
