...
// PRD §14.2 개인정보처리방침 v1.2 — 탈퇴 파기 정책 + 일반 사용자 언어로 순화 (2026-09-05). 정식 본문은 법무·DPO 검토 후 확정.
      ...
      <article className="prose prose-sm max-w-none space-y-4 text-sm text-body">
        <section>
          ...
          <ul className="list-disc pl-5">
            <li>소셜 계정 연결 정보(카카오·구글 등)·이메일·닉네임 — 로그인·계정 식별</li>
            ...
            <li>방문 인증 사진과 사진에 담긴 촬영 위치·시각 — 실제 방문 여부 확인(조작 방지)</li>
            <li>이메일 알림 설정(수신 여부·시간·요일) — 동의한 회원에게 추천 메일 발송</li>
            <li>동의 이력(동의 종류·버전·시각·IP 주소·브라우저 정보) — 동의 사실 증빙</li>
            <li>서비스 이용 기록·접속 IP — 보안, 과도한 요청 차단, 오류 분석, 이용 통계</li>
          </ul>
        </section>
        <section>
          <h2 className="font-semibold text-ink">2. 쿠키 및 유사 기술</h2>
          <ul className="list-disc pl-5">
            <li>로그인 상태 유지 쿠키 — 1시간 후 자동 만료, 스크립트로 읽을 수 없게 보호됨</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-5">
            <li>
              회원 탈퇴 시 이메일·소셜 계정 연결·닉네임·기준 주소와 좌표를 즉시 삭제하고, 반려견
              정보·북마크·추천 이력·방문 인증과 그 사진을 함께 파기합니다.
            </li>
            <li>
              작성하신 후기와 첨부 사진은 다른 이용자를 위해 남습니다. 신원 정보가 지워져 작성자를
              알 수 없는 익명 게시물이 되며(탈퇴 전 직접 삭제 가능), 개인을 식별할 수 있는 정보는
              남지 않습니다.
            </li>
            <li>동의 이력의 IP·접속 기기 정보: 탈퇴 30일 후 삭제 (동의 사실과 약관 버전은 보존)</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-5">
            ...
            <li>Upstash (과도한 요청 차단·임시 저장) — IP·요청 수 처리</li>
            <li>Resend (이메일 발송) — 미국</li>
            <li>카카오 (지도·길찾기·이동시간 조회) — 출발지·목적지 좌표가 API 호출에 사용됩니다</li>
            <li>Cloudflare (자동 입력 방지) — 글로벌</li>
            <li>Sentry (오류 모니터링) — 개인정보 자동 제거 후 수집</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-5">
            ...
            <li>회원 탈퇴: 마이펫타임 하단 {"'회원탈퇴'"}에서 직접 진행</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-5">
            <li>모든 통신 구간 암호화</li>
            <li>
              본인 데이터에만 접근 가능하도록 데이터베이스 수준에서 통제, 펫 헬스·좌표는 별도 암호화
            </li>
            <li>로그인 세션은 1시간 후 자동 만료</li>
            <li>입력 내용 검증·정화, 자동 입력(봇)·과도한 요청 차단</li>
