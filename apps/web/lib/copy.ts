...
export const COPY = {
  ...
  home: {
    ...
    card: {
      ...
      detail: '상세 보기',
      save: '저장',
      // Edge 응답 badges[] 라벨 — PET_VERIFIED 만 사용자 인증 기반, 나머지는 데이터 메타
      badge: {
        PET_VERIFIED: '검증 배지',
        WELLNESS: '웰니스',
        ECO: '생태',
        TRAIL_OFFICIAL: '두루누비 공식',
    ...
    // 플로팅 버튼 → 설명 모달 (PRD §6.3 · UI 명세 4)
    badgeGuide: {
      title: '검증 배지는 이렇게 붙어요',
      desc: '한국관광공사 데이터에 더해, 실제로 다녀온 분들의 인증이 쌓이면 장소에 검증 배지가 붙어요.',
      rules: [
        '서로 다른 방문자 3명 이상이 방문 인증',
        '사진의 촬영 위치가 장소 1km 이내, 촬영일이 7일 이내',
        '최근 6개월 안의 인증만 집계 · 6개월 뒤 자동 갱신',
      ] as string[],
      how: '장소 상세의 "방문 인증"에서 사진 한 장으로 참여할 수 있어요.',
      close: '닫기',
  ...
  trails: {
    ...
    // 경로 미리보기 (DB path_geojson → 카카오맵 Polyline / SVG 스케치)
    preview: '미리보기',
    previewAria: (name: string) => `${name} 경로 미리보기`,
    previewClose: '닫기',
    previewNoKey: '경로 형태만 보여드려요. 방향과 모양 참고용이에요.',
    pointCount: (n: number) => `좌표 ${n}개`,
    sketchAria: '코스 경로 스케치',
    mapLoadError: '지도를 불러오지 못했어요. 잠시 후 다시 시도해주세요.',
    start: '출발',
    end: '도착',
    distance: '거리',
    duration: '소요',
  ...
  upload: {
    ...
    uploading: '업로드 중…',
    remove: '사진 삭제',
    invalidType: '이미지 파일만 올릴 수 있어요.',
    tooLarge: '10MB 이하 이미지만 올릴 수 있어요.',
  ...
  poi: {
    ...
    verifiedBadge: '검증 배지',
    verifiedBadgeGranted: '검증 배지 획득 — 실제 방문자 3명 이상이 사진으로 인증한 곳이에요',
    verifyHint: '서로 다른 방문자 3명이 사진으로 인증하면 검증 배지가 붙어요',
    // 방문 인증 (체크인) 폼 — PRD §6.3
    checkin: {
      title: '방문 인증',
      desc: '다녀오셨나요? 그때의 한적함을 알려주고 사진 한 장으로 인증해주세요.',
      evaluationLabel: '얼마나 한적했나요?',
      evaluations: [
        { value: 'QUIET', label: '한적' },
        { value: 'OK', label: '보통' },
        { value: 'CROWDED', label: '복잡' },
      ] as Array<{ value: 'QUIET' | 'OK' | 'CROWDED'; label: string }>,
      photoLabel: '현장 사진 (검증에 필요)',
      photoHint: '사진의 촬영 위치가 장소 1km 이내, 촬영일이 7일 이내면 검증으로 집계돼요.',
      exifReading: '사진 정보 확인 중…',
      exifOk: (when: string) => `위치 정보 있음 · ${when} 촬영`,
      exifNoGps: '사진에 위치 정보가 없어요. 등록은 되지만 검증에는 집계되지 않아요.',
      exifNoDate: '위치 정보 있음 · 촬영일 없음',
      submit: '인증하기',
      submitting: '인증 중…',
      evaluationRequired: '한적함 정도를 선택해주세요',
      doneValid: '방문 인증 완료 — 검증에 집계됐어요',
      doneInvalid: '방문 인증 등록됨 — 사진 조건이 맞지 않아 검증에는 집계되지 않았어요',
      loginCta: '로그인하고 방문 인증',
      loginError: '로그인이 필요해요',
    },
    kakao: '카카오 길찾기',
    reviewsTitle: '방문 후기',
