...
- ⚠️ 현재 \`<SessionProvider>\`가 마운트돼 있지 않아 \`useSession()\`은 동작하지 않습니다.
  세션은 서버에서 \`auth()\`로 읽어 props로 내려주거나 \`GET /api/auth/session\`으로 조회하세요.
  ...
  paths: {
    ...
    '/api/auth/session': {
      get: {
        ...
        description:
          '**무엇**: 로그인 상태면 세션 JSON, 아니면 `{}`.\n\n**사용처**: 헤더의 로그인 상태 표시, 로그인 가드 등 **클라이언트에서 세션을 읽어야 할 때**. 현재 `<SessionProvider>`가 없어 `useSession()`이 동작하지 않으므로 이 엔드포인트를 fetch하거나, 서버에서 `auth()`로 읽어 props로 내려준다.',
        responses: {
          '200': {
